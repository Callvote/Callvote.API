...
namespace Callvote.API.Features.Votes
{
    ...
    public class Vote
    {
        ...
        /// <summary>
        /// Gets the <see cref="VoteOption"/>s that a <see cref="UserIndentifier"/> has voted on in a <see cref="Vote"/> .
        /// </summary>
        /// <param name="user">The user that is going to checked.</param>
        /// <returns>A collection of the voted options.</returns>
        public HashSet<VoteOption> GetVoteFromPlayer(UserIndentifier user) => this.PlayerVote.TryGetValue(user, out HashSet<VoteOption> votes) ? votes : [];
        ...
        internal CallVoteStatus StartVote()
        {
            ...
            VoteCalledEventArgs ev = new(this);
